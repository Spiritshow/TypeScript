...
interface User {
  ...
  function findInArray<T, K extends keyof T>(items: T[], keys: K |K[], value: T[K]): T | undefined {
    let keyArray = Array.isArray(keys) ? keys : [keys];
    return items.find(item => keyArray.some((key)=> item[key] === value))
  }
  ...
  const foundBook = findInArray(books,"title","Someone");

  const fb = findInArray(books,['author', 'title'],'Someone');
